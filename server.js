...
const authenticateToken = (req, res, next) => {
    ...
    try {
        ...
        // Attach the full user object from the token to the request
        req.user = decoded;
        next();
    } catch (error) {
...
// --- NEW ADMIN/OWNER AUTHENTICATION MIDDLEWARE ---
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        // Include roles in the JWT for middleware checks
        const token = jwt.sign({ id: user._id, username: user.username }, JWT_SECRET, { expiresIn: '24h' });
        res.json({ token });
    } catch (error) { res.status(500).json({ message: "Server error during login." }); }
});

// Profile endpoint now returns admin/owner status
...
// --- NEW ADMIN ROUTES ---
app.post('/api/admin/claim-daily', authenticateToken, adminAuthenticate, async (req, res) => {
    try {
        const user = await User.findById(req.user.id);
        const twentyFourHours = 24 * 60 * 60 * 1000;
        if (user.lastDailyClaim && (new Date() - new Date(user.lastDailyClaim) < twentyFourHours)) {
            return res.status(400).json({ message: 'You have already claimed your daily reward.' });
        }
        user.coins += 100000;
        user.lastDailyClaim = new Date();
        await user.save();
        await broadcastOnlineUsers();
        res.json({ message: 'You have claimed 100,000 coins!', newBalance: user.coins, lastDailyClaim: user.lastDailyClaim });
    } catch (error) { res.status(500).json({ message: 'Server error claiming reward.' }) }
});

app.post('/api/admin/clear-chat', authenticateToken, adminAuthenticate, async (req, res) => {
    try {
        await Message.deleteMany({});
        io.emit('chat_cleared'); // Notify all clients to clear their chat
        res.json({ message: 'Chat has been cleared.' });
    } catch (error) { res.status(500).json({ message: 'Server error clearing chat.' }) }
});

app.post('/api/admin/timeout-user', authenticateToken, adminAuthenticate, async (req, res) => {
    try {
        const { username, duration } = req.body;
        if (!username || !duration) return res.status(400).json({ message: 'Username and duration are required.' });

        const targetUser = await User.findOne({ username: username.toLowerCase() });
        if (!targetUser) return res.status(404).json({ message: 'User not found.' });
        if (targetUser.isOwner || targetUser.isAdmin) return res.status(403).json({ message: 'Cannot timeout another admin or owner.' });

        targetUser.timeoutUntil = new Date(Date.now() + duration * 60 * 1000);
        await targetUser.save();
        res.json({ message: `${username} has been timed out for ${duration} minutes.` });
    } catch (error) { res.status(500).json({ message: 'Server error timing out user.' }) }
});

// --- NEW OWNER ROUTES ---
app.post('/api/owner/give-coins', authenticateToken, ownerAuthenticate, async (req, res) => {
    try {
        const { username, amount } = req.body;
        const parsedAmount = parseInt(amount, 10);
        if (!username || isNaN(parsedAmount)) return res.status(400).json({ message: 'Username and a valid amount are required.' });
        ...
        await broadcastOnlineUsers();
        res.json({ message: `Successfully gave ${parsedAmount.toLocaleString()} coins to ${username}. Their new balance is ${targetUser.coins.toLocaleString()}.`});
    } catch (error) { res.status(500).json({ message: 'Server error giving coins.' }) }
});

app.post('/api/owner/set-admin', authenticateToken, ownerAuthenticate, async (req, res) => {
    try {
        const { username, isAdmin } = req.body;
        if (!username || typeof isAdmin !== 'boolean') return res.status(400).json({ message: 'Username and isAdmin status (true/false) are required.' });
        ...
        res.json({ message: `${username}'s admin status has been set to ${isAdmin}.` });
    } catch (error) { res.status(500).json({ message: 'Server error setting admin status.' }) }
});
...
io.use(async (socket, next) => {
    ...
    try {
        const decoded = jwt.verify(token, JWT_SECRET);
        const user = await User.findById(decoded.id).select('username timeoutUntil').lean(); // Fetch timeout status
        if (!user) return next(new Error("User not found"));
        socket.userId = user._id.toString();
        socket.username = user.username;
        socket.timeoutUntil = user.timeoutUntil;
        next();
...
io.on('connection', async (socket) => {
    ...
    try {
        ...
        const messagesWithPfps = await Promise.all(lastMessages.map(async msg => { const sender = await User.findOne({ username: msg.username }).select('pfp').lean(); return { ...msg, pfp: sender ? sender.pfp : 'https://i.imgur.com/8bzvETr.png' }; }));
        socket.emit('chat_history', messagesWithPfps.reverse());
    } catch (err) { console.error("Error during socket connection setup:", err); }

    socket.on('chat_message', async (msg) => {
        // --- UPDATED CHAT LOGIC WITH TIMEOUT CHECK ---
        const user = await User.findById(socket.userId).select('timeoutUntil');
        if (user.timeoutUntil && new Date(user.timeoutUntil) > new Date()) {
            return socket.emit('chat_error', 'You are currently timed out and cannot send messages.');
        }
    ...
    socket.on('delete_message', async (messageId) => {
        try {
            const requester = await User.findById(socket.userId);
            const message = await Message.findById(messageId);
            if (!message) return;
            
            // Allow deletion if user is admin/owner OR it's their own message within 5 mins
            const isOwnMessage = message.username === socket.username;
            const fiveMinutes = 5 * 60 * 1000;
            const isWithinTime = Date.now() - message.timestamp.getTime() < fiveMinutes;

            if (requester.isAdmin || requester.isOwner || (isOwnMessage && isWithinTime)) {
