...
app.post('/api/admin/clear-chat', authenticateToken, adminAuthenticate, async (req, res) => { try { await Message.deleteMany({}); io.emit('chat_cleared'); res.json({ message: 'Chat has been cleared.' }); } catch (error) { res.status(500).json({ message: 'Server error clearing chat.' }) }});
app.post('/api/admin/timeout-user', authenticateToken, adminAuthenticate, async (req, res) => { try { const { username, duration } = req.body; if (!username || !duration) return res.status(400).json({ message: 'Username and duration are required.' }); const targetUser = await User.findOne({ username: username.toLowerCase() }); if (!targetUser) return res.status(404).json({ message: 'User not found.' }); if (targetUser.isOwner || targetUser.isAdmin) return res.status(403).json({ message: 'Cannot timeout another admin or owner.' }); targetUser.timeoutUntil = new Date(Date.now() + duration * 60 * 1000); await targetUser.save(); res.json({ message: `${username} has been timed out for ${duration} minutes.` }); } catch (error) { res.status(500).json({ message: 'Server error timing out user.' }) }});
